...
/// @brief Reads the status and the headers (discarding the headers in the
...
/// @param handle The handle (http_read_status_and_headers must have already
/// been called on it)
...
char* http_url_encode(char* enc, size_t size, const char* s, char* table);
#ifdef __cplusplus
}
#endif
#endif  // HTTP_H